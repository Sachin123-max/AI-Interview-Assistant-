...
function Report({ report }) {
  //if report is null or undefined then show loading
  if (!report) {
    return (
  ...
  if (finalScore >= 8) {
    performanceText = "Ready for job opportunties.";
    shortTagline = "Excellent clarity and structured response.";
  } else if (finalScore >= 5) {
    performanceText = "Needs minor improvement before interviews.";
    shortTagline = "Good foundation,refine articulation.";
  } else {
    performanceText = "Significant imporovement required.";
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`onfidence: ${confidence}`,margin + 10, currentY + 10);
    ...
    let advice = "";
    if(finalScore >= 8){
      advice = "Excellent performance. Maintain confidence and structure";
    }else if(finalScore >= 5){
      advice = "Good Foundation shown. Improve Clarity"
    }else{
      advice = "Significant improvement required. focus on Structured"
    }
    ...
    //=======================================Qusetion Table============================
    autoTable(doc,{startY:currentY,margin:{left:margin, right:margin},
    head:[["#","Question","Score","Feedback"]],
  body:questionWiseScore.map((q,i)=> [`${i + 1}`,q.question,`${q.score}/10`,q.feedback,]),styles:{fontSize:9,cellPadding:5,valign:"top",},headStyles:{fillColor:[34,197,94],textColor:255,halign:"center"},columnStyles:{
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-green-50 px-4 sm:px-6 lg:px-10 py-8">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="mb-10 w-full flex items-start gap-4 flex-wrap">
          ...
          <div>
            <h1 className="text-3xl font-bold flex-nowrap text-gray-900">
              Interview Analytics Dashboard
            </h1>
            <p className="text-gray-600 mt-2">
              AI-powered performance insights and personalized feedback for
              interview success
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="space-y-6">
          ...
          >
            <h3 className="text-gray-500 mb-4 sm:mb-6 text-sm sm:text-blue-400">
              Overall Performance
            </h3>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }} className=" bg-white rounded-2xl sm:rounded-3xl shadow-lg p-5 sm:p-8">
