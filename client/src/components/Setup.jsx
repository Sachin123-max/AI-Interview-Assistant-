...
function Setup({onStart}) {
  ...
  const handleStart = async ()=> {
    ...
    try {
      ...
      console.error(error);
      setErrorMsg(error.response?.data?.message || 'Failed to start interview. Please check your inputs.');
      setLoading(false);
    }
  }
  ...
  return (
    ...
    className='min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 px-4'>
      <div className='w-full max-w-6xl bg-white rounded-3xl shadow-2xl grid md:grid-cols-2 overflow-hidden'>
        ...
        className='relative bg-gradient-to-br from-green-700 to-grren-100 p-12 flex flex-col justify-center'>

        <h2 className='text-4xl font-bold text-gray-800 mb-6'> Start your AI Interview</h2>
        <p className='text-gray-600 mb-10'>Practice real interview scenarios powerd by AI. Improve communication, technical skills, and confidence.</p>
        ...
        className='p-12 bg-white'>
          ...
          <div className='spavce-y-6'>
            ...
            <select value={mode}  onChange={(e)=>setMode(e.target.value)} children ='w-full py-3 px-4 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition'>
              ...
              {!analysisDone && (
                ...
                className='border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-green-500 hover:bg-green-50 transition'>
                  ...
                  <p className='text-gray-700 font-medium'>{resumeFile ? resumeFile.name : "Click to upload resume (Optional)"}</p>
                  {resumeFile && (
                    <motion.button 
                    whileHover={{scale:1.02}} onClick={(e)=>{e.stopPropagation();
                      handleUploadResume()}}
              ...
              onClick={handleStart}
               ...
               whileHOver={{scale:1.03}}
               whileTap={{scale:0.95}}
               className='w-full disabled:bg-gray-600 disabled:cursor-not-allowed bg-green-600 hover:bg-green-700 text-white py-3 rounded-full text-lg font-semibold
               transition duration-300 shadow-md'>{loading ? "Starting...":"Start Interview"} 
              </motion.button>
          </div>
        </motion.div>
      </div>
