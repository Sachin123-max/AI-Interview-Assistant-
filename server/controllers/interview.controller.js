...
export const analyzeResume = async (req, res) => {
  try {
    ...
    const messages = [
      {
        role: "system",
        content: `Extract structuree data from resume. Return strictly JSON: {
                "role:"string ,
                "experience":"string",
...
export const getMyInterviews = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `failed to find currentUser  interviews ${error}` });
  }
};
...
export const getInterviewReport = async (req, res) => {
  try {
    ...
    return res.status(500).json({message: `failed to find currentUser interviews report ${error}`})
  }
};
